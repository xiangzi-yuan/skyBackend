...
public interface DishMapper {
         ...
         */
        List<DishDetailRM> getByCategoryId(@Param("categoryId") Long categoryId, @Param("status") Integer status,
         ...
         * 根据状态统计菜品数量（排除已删除）
         */
        @Select("select count(id) from dish where status = #{status} and is_deleted = 0")
